...
export class ConfigFileWatcher extends EventEmitter {
  ...
  private analyzeEnvChange(
    filePath: string,
    changeType: ConfigChange['changeType'],
    change: ConfigChange
  ): ConfigChange {
  ...
  private calculateChangeImpact(
  ...
  ): { severity: ConfigChange['severity']; affectedServices: string[] } {
    const severityRules = {
    ...
    for (const key of changedKeys) {
      ...
      for (const [level, rules] of Object.entries(severityRules)) {
        if (rules.some((rule) => (rule.includes('*') ? key.endsWith(rule.slice(1)) : rule === key))) {
          if (level === 'critical') {
            severity = 'critical';
  ...
  private analyzeRuleChange(
    _filePath: string,
    _changeType: ConfigChange['changeType'],
    change: ConfigChange
  ): ConfigChange {
    return {
      ...change,
