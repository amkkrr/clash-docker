...
export class SmartRestartController extends EventEmitter {
  ...
  private async preRestartHealthCheck(serviceName: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Pre-restart health check failed for ${serviceName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async sendConfigReloadSignal(serviceName: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to send reload signal to ${serviceName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
