...
class HotReloadApp {
  ...
  public async start(): Promise<void> {
    try {
      ...
      // 初始化重启控制器
      this.restartController = new SmartRestartController(
        process.env.COMPOSE_PROJECT_NAME || 'clash-docker'
      );
  ...
  private setupConfigWatcherEvents(): void {
    ...
    this.configWatcher.on('configChange', async (change) => {
      try {
        ...
        this.broadcaster!.broadcastError('Config change handling failed', error instanceof Error ? error.message : 'Unknown error');
      }
    });
    ...
    this.configWatcher.on('error', (error) => {
      ...
      this.broadcaster!.broadcastError('Config watcher error', error instanceof Error ? error.message : 'Unknown error');
    });
  }
  ...
  private setupRestartControllerEvents(): void {
    ...
    // 监听服务重启完成事件
    this.restartController.on('serviceRestarted', ({ service, status, duration: _duration }) => {
      this.broadcaster!.broadcastRestartProgress(service, status);
    });
